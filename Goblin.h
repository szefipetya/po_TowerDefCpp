...
public:
	...
	Goblin(std::function<void(Entity*)> tick, std::function<void(Entity*)> death, int x, int y) : Unit(tick, death, x, y)
	{
		if (!canCreate()) {
			this->~Goblin();
			return;
		...
		this->dmg = 80;
		this->size = 1;
		this->virtual_scale = 1.3;
		this->speed = 1.8;
		this->aps = 1.3;
		this->type = Entity::Type::Goblin;
		status = Status::walk_north;
		count++;

	}

	Goblin(std::function<void(Entity*)> tick, std::function<void(Entity*)> death, int x, int y, UnitMetaData data[4][3], Entity::Difficulty diff) : Unit(tick, death, x, y)
	{
		if (!canCreate()) {
			this->~Goblin();
			return;
